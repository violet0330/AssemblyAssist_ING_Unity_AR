    ...
    public class Interaction : MonoBehaviour
    {
        ...
        private void UpdateGUI()
        {
            ...
            if (guiHandCursor)
            {
                ...
                {
                    if (lastLeftHandEvent == HandEventType.Grip)
                    { cursorTexture = gripHandTexture; GlobalVariables.handEvent = 1; }
                    else if (lastLeftHandEvent == HandEventType.Release)
                    { cursorTexture = releaseHandTexture; GlobalVariables.handEvent = 0; }
                }
                ...
                if ((cursorTexture != null) /**&& (isLeftHandPrimary || isRightHandPrimary)*/)
                {
                    ...
                    GlobalVariables.pointingPos = cursorScreenPos;

                    if (cursorProgressBar)
                    {
                        cursorProgressBar.fillAmount = cursorClickProgress;
                        GlobalVariables.clickProgress = cursorClickProgress;
                        if (cursorClickProgress > 0.8)
                        {
                            GlobalVariables.handEvent = 1;
                        }
                    }
